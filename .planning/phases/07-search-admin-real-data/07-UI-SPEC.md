...
| Section heading | `--text-title-lg` | 22px (1.375rem) | 600 (`--weight-semibold`) | 1.5 (`--leading-normal`) | Modal title (`h3`), order detail section headings (`h4`), page section titles |
| Table header | `--text-label-md` | 12px (0.75rem) | 500 (`--weight-medium`) | 1.5 (`--leading-normal`) | `<th>` — uppercase + `--tracking-wide` (0.02em) |
...
**Quy tắc:** Đúng 2 font weights active trên cùng 1 table row — `400 (--weight-regular)` cho data cells và `600 (--weight-semibold)` cho giá tiền / highlight cells. Không dùng weight 500 trực tiếp trên nội dung table.
...
|---|--------|-----------|-------------|
| 1 | Sản phẩm | ~35% | Thumbnail 48×48px (border-radius `--radius-md`) + product name (`--weight-medium`) + brand caption (`--text-body-sm`, `--outline`) |
| 2 | Danh mục | ~15% | Category name string |
| 3 | Giá | ~12% | `formatPrice(price)` — color `--primary`, weight `--weight-semibold` |
| 4 | Tồn kho | ~10% | Stock number; nếu `stock < 10` → color `--error`, weight `--weight-semibold` |
| 5 | Trạng thái | ~13% | `<Badge>` theo product status (xem bảng badge colors trên) |
| 6 | Thao tác | ~15% | Icon button ✏️ (edit) + 🗑️ (delete) — flex gap `--space-2` |

### Column Spec: Admin Orders Table

| # | Header | Width hint | Content spec |
|---|--------|-----------|-------------|
| 1 | Mã đơn | ~12% | `orderCode` — weight `--weight-semibold` (class `.tdBold`) |
...
| 4 | Tổng tiền | ~13% | `formatPrice(totalAmount)` — color `--primary`, weight `--weight-semibold` |
...
| 7 | Thao tác | ~15% | Icon button 📋 (xem chi tiết) → `router.push('/admin/orders/${order.id}')` |

### Column Spec: Admin Users Table

| # | Header | Width hint | Content spec |
|---|--------|-----------|-------------|
| 1 | Họ tên | ~20% | `fullName` nếu có và không rỗng, fallback: `username` — weight `--weight-semibold` |
...
[Line Items card — full width]
  background: --surface-container-lowest, border-radius: --radius-xl, padding: --space-5
  h3: Sản phẩm (--text-title-lg, --weight-semibold)
...
[Status Update card — full width]
  background: --surface-container-lowest, border-radius: --radius-xl, padding: --space-5
  h3: Cập nhật trạng thái (--text-title-lg, --weight-semibold)
...
Layout empty state:
```
flex column, align-items center, gap --space-3, padding --space-8
