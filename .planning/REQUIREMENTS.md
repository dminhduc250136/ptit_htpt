...
- [x] **PAY-02** — Return URL: VNPay redirect khách về `vnp_ReturnUrl` của ứng dụng sau thanh toán. FE render trang kết quả rõ ràng (thành công / thất bại / huỷ) dựa trên `vnp_ResponseCode`. KHÔNG cập nhật trạng thái đơn dựa trên return URL (return URL chỉ để hiển thị — nguồn sự thật là IPN).
