...
import com.ptit.htpt.inventoryservice.web.TraceIdFilter;
...
public class GlobalExceptionHandler {
  ...
  /**
   * Phase 24 (D-14): postgres-inventory down → 503 DATABASE_UNAVAILABLE.
   */
  @ExceptionHandler({
  ...
  public org.springframework.http.ResponseEntity<ApiErrorResponse> handleDatabaseUnavailable(
    ...
    return org.springframework.http.ResponseEntity.status(status).body(ApiErrorResponse.of(
        status.value(),
        status.getReasonPhrase(),
        "Inventory database is temporarily unavailable",
